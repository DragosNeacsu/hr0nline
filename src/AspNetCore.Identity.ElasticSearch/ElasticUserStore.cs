...
namespace AspNetCore.Identity.ElasticSearch
{
...

	{
		...
		public async Task<TUser> FindByIdAsync(string userId, CancellationToken cancellationToken)
		{
			...
			var esResult = await _nestClient.GetAsync<TUser>(userId, r => r
				.Index(Options.UsersIndex)
				, cancellationToken
			);
						...
						.Must(
							m => m.Nested(n => n
								  ...
								  .Query(qq => qq
									  .Bool(bb => bb
										  .Must(
											  mm => mm.Term(_nestClient.Infer.NestedProperty<ElasticUser, ElasticUserLogin>(x => x.Logins, x => x.LoginProvider), loginProvider),
											  mm => mm.Term(_nestClient.Infer.NestedProperty<ElasticUser, ElasticUserLogin>(x => x.Logins, x => x.ProviderKey), providerKey)
										  )
									  )
								  )
							),
						...
						.Must(
							m => m.Nested(n => n
								  ...
								  .Query(qq => qq
									  .Bool(bb => bb
										  .Must(
											  mm => mm.Term(_nestClient.Infer.NestedProperty<ElasticUser, ElasticClaim>(x => x.Claims, x => x.Type), claim.Type),
											  mm => mm.Term(_nestClient.Infer.NestedProperty<ElasticUser, ElasticClaim>(x => x.Claims, x => x.Value), claim.Value)
										  )
									  )
								  )
							),
		...
		public async Task<IList<TUser>> GetUsersInRoleAsync(string roleName, CancellationToken cancellationToken)
		{
			...
			var esUsers = await _nestClient.GetManyAsync<TUser>(esUserIds.Documents.Select(x => x.Id), Options.UserRolesIndex, Options.UsersIndex, cancellationToken);
